...
const RecipeDetailScreen: React.FC<RecipeDetailScreenProps> = ({ recipe, onBack, onToggleFavorite }) => {
  return (
    <div className="relative flex min-h-screen w-full flex-col bg-white dark:bg-background-dark">
      ...
      <div className="relative w-full h-80">
        ...
        <div className="absolute top-0 left-0 right-0 flex justify-between items-center p-4">
          ...
          <button 
            ...
            className={`flex items-center justify-center size-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 transition-colors ${
              recipe.isFavorite ? 'text-primary' : 'text-white'
            }`}
          >
      ...
      {/* Recipe Content */}
      <div className="relative -mt-8 rounded-t-3xl bg-white dark:bg-background-dark px-6 pt-8 pb-32 shadow-2xl">
        {/* Title and Rating */}
        ...
        <div className="mb-8">
          ...
          <ul className="space-y-3">
            {recipe.ingredients.map((ing, idx) => (
              <li key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
      ...
      {/* Floating CTA Button */}
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 bg-white/80 dark:bg-background-dark/80 backdrop-blur-lg z-50">
        <button 
